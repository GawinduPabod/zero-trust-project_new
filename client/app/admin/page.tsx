...
export default function AdminDashboard() {
  ...
  const handleApproveUser = async (email: string) => {
    await fetch(`${API_URL}/admin/user/action`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, action: 'approve' }) });
    fetchData();
  };

  const handleKickUser = async (email: string) => {
    await fetch(`${API_URL}/admin/user/action`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, action: 'kick' }) });
    alert(`User ${email} kicked successfully!`); fetchData();
  };

  const handleLockUser = async (email: string, currentStatus: boolean) => {
    await fetch(`${API_URL}/admin/user/action`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, action: currentStatus ? 'unlock' : 'lock' }) });
    fetchData();
  };
  ...
  const handleLockdown = async () => {
    ...
    const res = await fetch(`${API_URL}/admin/system/lockdown`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ state: newState }) });
    alert((await res.json()).message);
    setIsLockdown(newState); fetchData();
  };

  const handleSimulateAttack = async () => {
    if (!window.confirm("Simulate DDoS Attack?")) return;
    alert("Attack started! Sending requests...");
    for (let i = 0; i < 20; i++) fetch(`${API_URL}/login`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: `fake_hacker_${i}@gmail.com`, username: "hacker", location: "Unknown" }) }).catch(() => {});
    setTimeout(() => { alert("Attack blocked! Check Security Alerts."); fetchData(); }, 3000);
  };

  // ==========================================
  // NEW: DOWNLOAD USER DATA AS CSV
  // ==========================================
  const downloadCSV = () => {
  ...
  // ==========================================
  // NEW: DOWNLOAD SECURITY AUDIT REPORT
  // ==========================================
  ...
  const handleChatSubmit = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && command.trim() !== '') {
      ...
      setChatHistory(newChat); setCommand('');
      try {
        const res = await fetch(`${API_URL}/admin/copilot/chat`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: command }) });
        setChatHistory([...newChat, { sender: 'AI', text: (await res.json()).response }]);
      } catch { setChatHistory([...newChat, { sender: 'AI', text: 'ERROR: CONNECTION FAILED.' }]); }
    }
  };
    ...
    <div style={{ backgroundColor: '#000', color: '#0ff', minHeight: '100vh', padding: '20px', fontFamily: 'monospace' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #0ff', paddingBottom: '10px', marginBottom: '20px' }}>
        ...
        <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
            <button onClick={handleSimulateAttack} style={{ padding: '8px 15px', backgroundColor: '#500', color: '#fff', border: '1px solid #f00', cursor: 'pointer', fontWeight: 'bold' }}>⚠️ SIMULATE DDoS</button>
            <button onClick={handleLockdown} style={{ padding: '8px 15px', backgroundColor: isLockdown ? '#006400' : '#800000', color: '#fff', border: `1px solid ${isLockdown ? '#0f0' : 'red'}`, cursor: 'pointer', fontWeight: 'bold' }}>{isLockdown ? '✅ LIFT LOCKDOWN' : '⚠️ INITIATE LOCKDOWN'}</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '20px' }}>
        <div style={{ flex: 2, display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              ...
              <button onClick={downloadCSV} style={{ background: '#022', color: '#0ff', border: '1px solid #0ff', padding: '5px 10px', cursor: 'pointer' }}>⬇️ Export Users (CSV)</button>
            </div>
          ...
          <div style={{ marginTop: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid red', paddingBottom: '5px', marginBottom: '10px' }}>
              ...
              <button onClick={downloadAuditReport} style={{ background: '#300', color: '#fdd', border: '1px solid red', padding: '5px 10px', cursor: 'pointer' }}>📄 Download Audit Log</button>
            </div>
            <div style={{ height: '250px', overflowY: 'auto', border: '1px solid #800', backgroundColor: '#100000', padding: '15px' }}>
              {securityLogs.length === 0 ? <p style={{ color: '#0f0', margin: 0 }}>[+] System is secure. No recent breaches detected.</p> : securityLogs.map((log, i) => (
        ...
        <div style={{ flex: 1, border: '1px solid #088', display: 'flex', flexDirection: 'column', height: '80vh' }}>
          <div style={{ padding: '10px', backgroundColor: '#022', borderBottom: '1px solid #088', display: 'flex', justifyContent: 'space-between' }}><strong>SECURITY_COPILOT</strong><span style={{ fontSize: '12px', color: '#088' }}>[AI MODULE]</span></div>
          <div style={{ flex: 1, padding: '15px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '15px' }}>
            {chatHistory.map((chat, idx) => (
              <div key={idx} style={{ alignSelf: chat.sender === 'ADMIN' ? 'flex-end' : 'flex-start', maxWidth: '80%' }}>
                ...
                <div style={{ border: '1px solid #088', padding: '10px', backgroundColor: chat.sender === 'ADMIN' ? '#011' : 'transparent', color: '#0ff' }}><pre style={{ margin: 0, whiteSpace: 'pre-wrap', fontFamily: 'monospace' }}>{chat.text}</pre></div>
              </div>
            ))}
          </div>
          <div style={{ padding: '10px', borderTop: '1px solid #088', display: 'flex', alignItems: 'center' }}>
            <span style={{ color: '#0f0', marginRight: '10px' }}>{'>'}</span>
            <input type="text" value={command} onChange={(e) => setCommand(e.target.value)} onKeyDown={handleChatSubmit} placeholder="Enter command..." style={{ background: 'transparent', border: 'none', color: '#0ff', width: '100%', outline: 'none', fontFamily: 'monospace' }} />
          </div>
        </div>
